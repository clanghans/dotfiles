...
while true; do
    ...
    if timeout 5 nslookup example.com 10.x.x.x &>/dev/null || \
       timeout 5 nslookup example.com 10.x.x.x &>/dev/null || \
       timeout 5 nslookup google.com 8.8.8.8 &>/dev/null; then
        ...
        if [ $FAIL_COUNT -ge $FAIL_THRESHOLD ]; then
            ...
            nmcli connection down "REDACTED-IFACE" 2>&1 >> "$LOGFILE"
            sleep 2
            nmcli connection up "REDACTED-IFACE" 2>&1 >> "$LOGFILE"
