...
hl.bind("CTRL + ALT + DELETE", hl.dsp.exec_cmd("~/.config/hypr/scripts/window_close_all.sh"), { description = "Close all windows" })
...
hl.bind("SUPER + CTRL + F", hl.dsp.exec_cmd("~/.config/hypr/scripts/window_tiled_fullscreen_toggle.sh"), { description = "Tiled full screen" })
hl.bind("SUPER + ALT + F", hl.dsp.window.fullscreen({ mode = "maximized" }), { description = "Full width" })
hl.bind("SUPER + O", hl.dsp.exec_cmd("~/.config/hypr/scripts/window_pop.sh"), { description = "Pop window out (float & pin)" })
hl.bind("SUPER + L", hl.dsp.exec_cmd("~/.config/hypr/scripts/workspace_layout_toggle.sh"), { description = "Toggle workspace layout" })
-- Dropped: save/restore window width (omarchy-hyprland-window-width). Minor
-- convenience, not part of the confirmed scope; add a vendored script here
...
-- Dropped: monitor scaling up/down (omarchy-hyprland-monitor-scaling). Its
-- laptop/audit-log logic doesn't fit this fixed dual-monitor desktop; set
-- scale directly in monitors.lua if a display's DPI needs to change.
